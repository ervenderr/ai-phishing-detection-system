...
export default function UserDashboard() {
  ...
  // Mock user ID for demo; replace with real auth in production
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        const threatsBlocked = notifications.filter(
          (n) => n.verdict === 'critical' || n.verdict === 'dangerous'
        ).length;
        const safeEmails = notifications.filter((n) => n.verdict === 'safe').length;
        const falsePositives = notifications.filter((n) =>
          n.description?.toLowerCase().includes('false positive')
        ).length;
        const protectionRate = totalEmails
          ? Math.round(((safeEmails + threatsBlocked) / totalEmails) * 100)
          : 100;
        setUserStats({
          totalEmails,
          threatsBlocked,
        ...
        // Recent alerts: show last 3, sorted by timestamp desc
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {userStats.safeEmails.toLocaleString()}
            </div>
          ...
          <CardContent>
            <div className="space-y-4">
              {recentAlerts.map((alert) => (
                <div
                  key={alert.id}
                  className={`p-4 border-l-4 rounded-lg ${getThreatColor(
                    alert.verdict === 'critical' || alert.verdict === 'dangerous'
                      ? 'high'
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="p-4 border rounded-lg">
              ...
              <p className="text-sm text-gray-600">
                Always verify the sender&apos;s identity before clicking links or downloading
                attachments, especially for urgent requests.
              </p>
            </div>
            <div className="p-4 border rounded-lg">
              <h4 className="font-medium mb-2">Check URLs Carefully</h4>
              <p className="text-sm text-gray-600">
                Hover over links to see the actual destination. Be wary of shortened URLs or
                suspicious domains.
              </p>
            </div>
            <div className="p-4 border rounded-lg">
              ...
              <p className="text-sm text-gray-600">
                Help improve our protection by reporting false positives and suspicious emails that
                weren&apos;t caught.
              </p>
            </div>
          </div>
